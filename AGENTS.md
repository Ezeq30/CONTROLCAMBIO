...
- `exportar_resumen_html` — incluye panel VALIDACIONES (Carrera/Caballos/Observación) junto a bases, misma altura (`align-items: stretch`). `exportar_comparacion_html` **no** incluye VALIDACIONES (solo consola).
...
- **HTML:** el resumen de tela incluye VALIDACIONES (Carrera/Caballos/Observación) a la derecha de Bases por apuesta, estirado a la misma altura. El HTML de comparación no las exporta (solo consola).
